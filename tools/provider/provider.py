...
def Read(json_path: str) -> Providers:
    with open(json_path, "r", encoding="utf-8") as file:
        data = json.load(file)

    cloudflare = None
    if "cf" in data:
        cloudflare_data = data["cf"]
        cloudflare = Cloudflare(
            ...
            SUBSCRIBE_USER_TOKEN=cloudflare_data["USER_TOKEN"],
        )

    subscribe_list = []
    for sub in data.get("subscription", []):
    ...
    for sort in data.get("sort", []):
        sort_item = Sort(rang=sort["range"], keywords=sort["keywords"])
